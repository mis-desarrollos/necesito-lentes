			...
			<div class="panel panel-primary" data-collapsed="0">
				...
				<div class="panel-body">
					<form role="form" class="form-horizontal" @submit.prevent="newRow($event.target)">
						...
						<input-form type="password" name="password_optica" text="Password" :data.sync="row.password"
							:validate="rule_password" place="Solo si desea cambiarla"></input-form>
						...
						<vue-dropzone ref="VueDropzone" id="dropzone" :options="dropzoneOptions"
							@vdropzone-complete="onUpload"></vue-dropzone>
						<br><br>
						<table v-if="row.images?.length > 0" id="tableproducts" aria-describedby="imagenes">
							<tr>
								<th>Imagen</th>
								<th>Eliminar</th>
							</tr>
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								<v-select v-model="row.states_id" :options="estados" label="name" index="id"
									@change="getTowns" required />
							</div>
						</div>
						...
						<div class="row">
							<p style="text-align:center;font-size: 17p;"><b>Da click sobre el mapa o arrastra el marcador
									para fijar la ubicacion</b></p>

							<div id="searchemap" v-if="mapLoaded">
								<b>Buscar Domicilio: </b><br>
								<vue-google-autocomplete id="map" classname="form-control" placeholder="Buscar..."
									v-on:placechanged="getAddressData" country="mx" :value="row.address"
							...
							<br>
							<div>
								<GmapMap :center="{ lat: row.lat, lng: row.lng }" :zoom="16" map-type-id="terrain"
									style=" height: 300px" @click="setPlace($event)">
									<GmapMarker :position="{ lat: row.lat, lng: row.lng }" :clickable="true"
						...
						<div class="form-group">
							<div class="col-sm-12">
								...
								<button type="button" class="btn btn-default pull-right"
									@click="$router.push('/opticians/')">Cancelar</button>
							</div>
						</div>
...
export default {
	data() {
		return {
			...
			mapLoaded: false,
			dropzoneOptions: {
				url: tools.url('/api/admin/dropzone/opticians/' + this.$route.params.id),
				acceptedFiles: 'image/*',
	...
	methods: {
		...
		deleteImage(id) {
			alertify.confirm("Alerta!", "¿Seguro que desea eliminar esta imagen?", () => {
				axios.delete(tools.url("/api/admin/dropzone/opticians/" + this.row.id + "/image/" + id)).then(result => {
					this.$parent.showMessage(result.data.message, "success");
					this.getRow();
				}).catch(error => {
					this.$parent.handleErrors(error);
				});
