									...
									<div class="form-group">
										<div class="col-sm-12">
											<button type="button" class="btn btn-danger" @click="deleteRow"
												v-show="$route.params.id"><i class="fa fa-trash"></i> Borrar</button>
											<button type="submit" class="btn btn-success pull-right"><i
													class="far fa-save"></i> Guardar</button>
											<button type="button" class="btn btn-default pull-right"
												@click="$router.push('/frames/')">Cancelar</button>
										</div>
									</div>
	...
	methods: {
		...
		newRow(form) {
			...
			this.$parent.validateAll(() => {
				const data = tools.params(form, this.row);
		...
		deleteImage(id) {
			alertify.confirm("Alerta!", "¿Seguro que desea eliminar esta imagen?", () => {
				axios.delete(tools.url("/api/admin/dropzone/framesImage/" + this.row.id + "/image/" + id)).then(result => {
