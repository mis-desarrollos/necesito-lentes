	...
	<div class="sidebar-menu" >
		...
		<div class="sidebar-menu-inner">
			...
			<ul id="main-menu" class="main-menu">
				...
				<li v-for="(item, index) in menu" :class="{'has-sub':item.sub_menus.length>0}" :key="index">
					<a v-if="item.sub_menus.length>0" style="color: white;"> 
						<i :class="item.icon"></i>
						<span class="title">{{item.name}}</span>
					</a>
					<router-link :to="item.route" v-else style="color: white;">
						<i :class="item.icon"></i>
						<span class="title">{{item.name}}</span>
					</router-link>
					<ul v-if="item.sub_menus.length>0">
						<li v-for="(sub, subindex) in item.sub_menus" :key="subindex" style="color: white;">
...
<script type="text/javascript">
	export default {
		...
		methods:{
			...
			getMenus(){
				axios.get(tools.url("/api/admin/menu")).then((response)=>{
