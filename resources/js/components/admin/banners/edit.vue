			...
			<div class="panel panel-primary" data-collapsed="0">
				...
				<div class="panel-body">
					<form role="form" class="form-horizontal" @submit.prevent="newBanners($event.target)">
						...
						<input-form name="posicion" text="Posición" type="number" :data.sync="banner.position" validate="required" v-if="banner.section == 0"></input-form>
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<label class="col-sm-12 control-label" style="text-align: left; padding-bottom: 10px" v-if="banner.section == 0">Dimensiones recomendadas (1920 x 562): Celular(375 x 300):</label>
								<label class="col-sm-12 control-label" style="text-align: left; padding-bottom: 10px" v-if="banner.section != 0">Dimensiones recomendadas (496 x 639)</label>
	...
	export default {
		...
		methods:{
			getBanners(){
				...
				axios.get(tools.url("/api/admin/banners/"+this.id)).then((response)=>{
			...
			newBanners(form){
				...
				this.$parent.validateAll(()=>{
					...
					if(this.$route.params.id){
						axios.post(tools.url("/api/admin/banners/"+this.id),data)
						.then((response)=>{
					    	this.getBanners();
						...
						axios.post(tools.url("/api/admin/banners"),data).then((response)=>{
			...
			deleteBanners:function(){
				alertify.confirm("Alerta!","¿Seguro que deseas borrar este banner?",()=>{
					this.$parent.inPetition=true;
					axios.delete(tools.url("/api/admin/banners/"+this.id))
					.then((response)=>{
